...
p_seird_dist <- df_dist %>%
  ...
  geom_line(aes(y = E * 10, color = "Exposed (scaled)")) +
  geom_line(aes(y = I * 10, color = "Infectious (scaled)")) +
  geom_line(aes(y = R, color = "Recovered")) +
  geom_line(aes(y = D, color = "Deaths"), linetype = "dashed") +
  scale_colour_manual(
    values = c(
      "Susceptible" = "steelblue", "Exposed (scaled)" = "orange",
      "Infectious (scaled)" = "black", "Recovered" = "green4", "Deaths" = "gray30"
    )
  ) +
...
cat("Susceptible (End)   :", final_S, "\n")
cat("Susceptible (%)     :", 100 * final_S / pars$N_ini, "%\n\n")
